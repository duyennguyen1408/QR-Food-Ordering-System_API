...
const genRandomString = function (length) {
    ...
    for (let i = 0; i < length; i++) {
        sResult += sPossible.charAt(
            Math.floor(Math.random() * sPossible.length)
        );
    }
    return sResult;
};
...
const VariableEnumChecking = function (value, arrayData) {
    ...
    if (!VariableBaseTypeChecking(value, "string")) {
        ...
        bResult = !(
            arrayData.indexOf(value) < 0 &&
            arrayData.indexOf(value.toUpperCase()) < 0
        );
    }
    return bResult;
};
...
const splitAndAssignValueForKey = function (
    availableObject,
    stringArray,
    assignValue
) {
        ...
        if (!objectArray) {
            ...
            for (i = 0; i < objectArray.length; i++) {
                if (
                    objectArray[i].id &&
                    element.id &&
                    objectArray[i].id.toString() === element.id.toString()
                ) {
                    break;
                }
            }
            ...
            for (i = 0; i < objectArray.length; i++) {
                if (key) {
                    ...
                    if (
                        objectArray[i] &&
                        element &&
                        objectArray[i].toString() === element.toString()
                    ) {
                        objectArray.splice(i, 1);
                        objectArray.push(element);
                        break;
        ...
        if (objectArray && objectArray.length > 0) {
            for (let i = 0; i < objectArray.length; i++) {
                ...
                if (
                    obj.id &&
                    elementId &&
                    obj.id.toString() === elementId.toString()
                ) {
                    objectArray.splice(i, 1);
                    i--;
                }
        ...
        if (objectArray && objectArray.length > 0) {
            for (let i = 0; i < objectArray.length; i++) {
                ...
                if (
                    obj[findKey] &&
                    findValue &&
                    obj[findKey].toString() === findValue.toString()
                ) {
...
const validAddingDevice = function (
    userId,
    userRight,
    payLevel,
    curDeviceNo,
    ...
    try {
        ...
        switch (userRight) {
            ...
            case "END_USER":
                switch (payLevel) {
                    case "COPPER":
                        if (expiredDate.getTime() <= curDate.getTime()) {
                            maxDevice = global.CLOUD_API.Config.MAX_DEVICE
                                ? global.CLOUD_API.Config.MAX_DEVICE
                                : 1;
                            if (curDeviceNo < maxDevice) {
                                result = true;
                            }
                        } else {
                            maxDevice = global.CLOUD_API.Config.MAX_DEVICE1
                                ? global.CLOUD_API.Config.MAX_DEVICE1
                                : 1;
                            if (curDeviceNo < maxDevice) {
                                result = true;
                            }
                    ...
                    case "SILVER":
                        if (expiredDate.getTime() <= curDate.getTime()) {
                            ...
                            maxDevice = global.CLOUD_API.Config.MAX_DEVICE2
                                ? global.CLOUD_API.Config.MAX_DEVICE2
                                : 1;
                            if (curDeviceNo < maxDevice) {
                                result = true;
                            }
                    ...
                    case "GOLD":
                        if (expiredDate.getTime() <= curDate.getTime()) {
                            ...
                            maxDevice = global.CLOUD_API.Config.MAX_DEVICE3
                                ? global.CLOUD_API.Config.MAX_DEVICE3
                                : 1;
                            if (curDeviceNo < maxDevice) {
                                result = true;
                            }
                    ...
                    case "DIAMOND":
                        if (expiredDate.getTime() <= curDate.getTime()) {
                            ...
                            maxDevice = global.CLOUD_API.Config.MAX_DEVICE4
                                ? global.CLOUD_API.Config.MAX_DEVICE4
                                : 1;
                            if (curDeviceNo < maxDevice) {
                                result = true;
                            }
        ...
        // demo, dohchung
        if (
            userId == "592d1d91b138731350b1d2ff" ||
